...
原方案日期：2026-09-24；实现状态更新：2026-09-25。当前任务清单见 [TODO](../TODO.md)，用户入口见 [README](../README.md)。本文明确区分已实现的有限能力、已完成的交付验收和未来研究。

## 1. 产品目标与本阶段结论

为下载源码自行编译或下载原生二进制的用户，提供能够解释检测范围的 MoonBit 安全检测工具。扫描自己的项目和第三方源码同等重要。分析器构建工具链固定，待检测项目工具链由用户选择；语言迭代导致的编译、解析或模型缺口分别报告。

**A–D 的有限实现已形成闭环，并通过 Linux 本地生产入口验收。** 默认是有限语法扫描；可选语义模式只覆盖明确声明的 mocket `.get` 回调及经核实的 cmark 模型。三平台真实 CI 与归档验收已经通过，记录见[平台证据](metrics/three-platform-acceptance-2026-09-25.json)；[草稿 PR #1](https://github.com/I3eg1nner/moon-audit/pull/1)尚未合并发布。
...
[Linux 生产入口 15 项验收](metrics/semantic-production-acceptance-2026-09-25.json)记录冷/热输出一致、baseline、模型失配、递归/展开、子进程故障、地址空间分配失败及超时子孙进程清理。固定小项目的冷/热运行约 6.01 / 5.11 秒；这个样本不能外推到任意规模项目。真实 macOS CI 发现的 worker 资源初始化失败已修复，采样内存超额与无存活子孙均已实测通过；Windows 长短路径身份及受限语义入口也已通过。Windows 专门提交内存超额注入尚未执行。
...
| D | 可选生产入口、统一证据/范围、预算与失败保留 | 三平台受限生产入口验收通过；未正式发行 |

三平台实现提交 `1a3e8b1` 的真实构建、解包与语义运行已全部通过[CI](https://github.com/I3eg1nner/moon-audit/actions/runs/36105436648)，常规构建和三个版本兼容任务也全部通过。归档、二进制 SHA 和平台范围见验收记录。实现、测试与文档通过 draft PR 交付审查；合并和正式发行不包含在本轮授权中。
