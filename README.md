...
对 MoonBit 生态 21 个开源项目（总计 3,676 个文件）的默认配置扫描结果：

| 项目 | Stars | 扫描文件 | 检出数 | 检出类型 |
|---|---|---|---|---|
| [moonbitlang/core](https://github.com/moonbitlang/core) | 1154 | 438 | 0 | 标准库白名单，跳过通用规则 |
| [mizchi/actrun](https://github.com/nicholasgasior/actrun) | 655 | 77 | 39 | abort (35)、unsafe 类型转换 (4) |
| [mizchi/luna.mbt](https://github.com/mizchi/luna.mbt) | 162 | 466 | 57 | CRLF 注入 (14)、abort (28)、unsafe 类型转换 (15) |
| [moonbit-community/rabbita](https://github.com/moonbit-community/rabbita) | 119 | 155 | 6 | abort (6) |
| [mizchi/markdown.mbt](https://github.com/mizchi/markdown.mbt) | 97 | 82 | 14 | abort (1)、unsafe 类型转换 (13) |
| [oboard/mocket](https://github.com/oboard/mocket) | 94 | 50 | 12 | CRLF 注入 (2)、abort (2)、Cookie (3)、模板注入 (1)、CORS (4) |
| [mizchi/js.mbt](https://github.com/mizchi/js.mbt) | 74 | 196 | 5 | abort (5) |
| [mizchi/crater](https://github.com/nicholasgasior/crater) | 69 | 742 | 127 | unsafe 类型转换 (127) |
| [moonbitlang/async](https://github.com/moonbitlang/async) | 65 | 188 | 14 | abort (11)、unsafe 类型转换 (3) |
| [mizchi/tornado](https://github.com/mizchi/tornado) | 62 | 26 | 0 | - |
| [mizchi/pkfire](https://github.com/mizchi/pkfire) | 48 | 18 | 10 | unsafe 类型转换 (10) |
| [moonbitlang/maria](https://github.com/moonbitlang/maria) | 43 | 266 | 17 | abort (11)、unsafe 类型转换 (6) |
| [moonbit-community/selene](https://github.com/moonbit-community/selene) | 41 | 368 | 18 | abort (18) |
| [justjavac/moonbit-webview](https://github.com/justjavac/moonbit-webview) | 39 | 31 | 1 | abort (1) |
| [extism/moonbit-pdk](https://github.com/extism/moonbit-pdk) | 38 | 19 | 2 | unsafe 类型转换 (2) |
| [moonbit-community/cmark.mbt](https://github.com/moonbit-community/cmark.mbt) | 34 | 51 | 20 | unsafe 类型转换 (19)、cmark XSS (1) |
| [moonbitlang/python.mbt](https://github.com/nicholasgasior/python.mbt) | 33 | 49 | 0 | - |
| [moonbitlang/openseek](https://github.com/nicholasgasior/openseek) | 32 | 291 | 5 | abort (1)、unsafe 类型转换 (4) |
| [moonbitlang/mooncakes.io](https://github.com/nicholasgasior/mooncakes.io) | 24 | 51 | 0 | - |
| [oboard/mio](https://github.com/oboard/mio) | 23 | 42 | 2 | abort (1)、unsafe 类型转换 (1) |
| [bobzhang/crescent](https://github.com/bobzhang/crescent) | 5 | 70 | 13 | abort (1)、Cookie (3)、DoS (5)、CORS (4) |

通用安全规则（CWE-676/248/704）覆盖所有应用项目，内置上下文感知过滤（FFI 绑定层、guard 校验、性能 unsafe 操作、平台桩文件、契约断言等安全模式自动跳过）；标准库模块（`moonbitlang/core`、`moonbitlang/x` 等）自动跳过通用规则（unsafe 调用和 abort 是标准库的实现细节）；Web 框架规则通过 Import 门控仅在相关依赖存在时激活。
...
- **标准库白名单**：自动识别 `moonbitlang/core`、`moonbitlang/x` 等标准库模块，跳过通用规则（unsafe 和 abort 是标准库实现细节）
- **上下文感知过滤**：自动识别 FFI 绑定、guard 校验、平台桩文件、契约断言等安全上下文，抑制已知安全模式的误报
...
| 规则 ID | 描述 | 上下文过滤 | 严重级别 |
|---|---|---|---|
| CWE-676/unsafe-call | 危险类型转换 (`unsafe_to_*`/`unsafe_from_*`/`unsafe_new`) | 性能操作 (get/set/blit/reinterpret) 跳过；guard body 跳过 | Warning |
| CWE-248/panic-reachable | 库代码中 `abort("message")` 使调用者无法恢复 | 裸 panic()、guard-else、平台桩文件、abort("unreachable")、契约断言 跳过 | Warning |
| CWE-704/unsafe-cast | `.cast()` 绕过类型系统，运行时可能产生无效值（默认关闭，FFI 项目误报率 ~98%） | FFI 绑定文件（extern "js" / #external）跳过 | Warning |
