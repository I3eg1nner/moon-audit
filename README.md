...
对 MoonBit 生态 21 个开源项目（总计 3,676 个文件）的默认配置扫描结果（通用规则默认关闭，仅 Web 安全规则）：

| 项目 | Stars | 扫描文件 | 检出数 | 检出类型 |
|---|---|---|---|---|
| [mizchi/luna.mbt](https://github.com/mizchi/luna.mbt) | 162 | 466 | 14 | CRLF 注入 (14) |
| [oboard/mocket](https://github.com/oboard/mocket) | 94 | 50 | 6 | CRLF 注入 (2)、Cookie (1)、CORS (3) |
| [bobzhang/crescent](https://github.com/bobzhang/crescent) | 5 | 70 | 8 | Cookie (1)、DoS (4)、CORS (3) |
| [moonbit-community/cmark.mbt](https://github.com/moonbit-community/cmark.mbt) | 34 | 51 | 1 | cmark XSS (1) |
| 其他 17 个项目 | — | 3,039 | 0 | - |

默认配置下共 **29 个 findings**，全部为 Web 安全规则检出，其中 17 个 Error 级别。通用规则（CWE-676/248/704）默认关闭，可通过 `.moon-audit.json` 或 `--rule` 按需启用。
...
| 规则 ID | 描述 | 上下文过滤 | 严重级别 |
|---|---|---|---|
| CWE-676/unsafe-call | 危险类型转换 (`unsafe_to_*`/`unsafe_from_*`/`unsafe_new`)。**默认关闭** | 性能操作 (get/set/blit/reinterpret) 跳过；guard body 跳过 | Warning |
| CWE-248/panic-reachable | 库代码中 `abort("message")` 使调用者无法恢复。**默认关闭** | 裸 panic()、guard-else、平台桩文件、abort("unreachable")、契约断言 跳过 | Warning |
| CWE-704/unsafe-cast | `.cast()` 绕过类型系统，运行时可能产生无效值。**默认关闭** | FFI 绑定文件（extern "js" / #external）跳过 | Warning |
...
```json
{
  "rules": {
    ...
    "CWE-248/panic-reachable": { "enabled": true },
    "CWE-704/unsafe-cast": { "enabled": true }
  }
}
```

> Import 门控原理：当项目未引入 `cmark`、`mocket`、`crescent`、`rabbita` 等包时，相关 Web 规则自动跳过，不会产生误报。通用规则（CWE-676/248/704）默认关闭以减少噪音，可按需启用进行深度审计。
