...
对 MoonBit 生态 15 个开源项目（总计 2,418 个文件）的全流程扫描结果：

| 项目 | Stars | 扫描文件 | 检出数 | 检出类型 |
|---|---|---|---|---|
| [moonbitlang/core](https://github.com/moonbitlang/core) | 1154 | 436 | 139 | abort (104)、unsafe 类型转换 (35) |
...
| [oboard/mocket](https://github.com/oboard/mocket) | 94 | 48 | 8 | CRLF 注入 (2)、abort (2)、Cookie (1)、CORS (3) |
| [mizchi/js.mbt](https://github.com/mizchi/js.mbt) | 74 | 196 | 60 | abort (5)、unsafe cast (55) |
| [moonbitlang/async](https://github.com/moonbitlang/async) | 65 | 187 | 15 | abort (12)、unsafe 类型转换 (3) |
...
| [moonbitlang/maria](https://github.com/moonbitlang/maria) | 43 | 266 | 21 | abort (13)、unsafe 类型转换 (6)、unsafe cast (2) |
| [moonbit-community/selene](https://github.com/moonbit-community/selene) | 41 | 368 | 20 | abort (20) |
...
| [moonbit-community/cmark.mbt](https://github.com/moonbit-community/cmark.mbt) | 34 | 51 | 21 | abort (1)、unsafe 类型转换 (19)、cmark XSS (1) |
| [bobzhang/crescent](https://github.com/bobzhang/crescent) | 5 | 69 | 9 | abort (1)、Cookie (1)、DoS (4)、CORS (3) |

通用安全规则（CWE-676/248/704）覆盖所有 MoonBit 项目，内置上下文感知过滤（FFI 绑定层、guard 校验、性能 unsafe 操作、平台桩文件等安全模式自动跳过）；Web 框架规则通过 Import 门控仅在相关依赖存在时激活。core/async 等基础库的检出以 abort 和 unsafe 类型转换为主，属于库代码设计决策（abort 而非 raise error），应用项目可通过 `.moon-audit.json` 按需配置。
...
| CWE-248/panic-reachable | 库代码中 `abort("message")` 使调用者无法恢复 | 裸 panic()、guard-else、平台桩文件、abort("unreachable") 跳过 | Warning |
