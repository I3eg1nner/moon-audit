...
## 2026-09-26：LLM 复核证据增强

- [x] 为已生成 IR 的函数/回调导出 AST 源码范围；不恢复旧求解器。
- [x] 按告警完整路径选择 route，沿 IR 调用边附完整辅助函数、单位身份、原始绑定及声明模型。
- [x] 附件 ID 限定到当前告警，跨文件精确引用；完整输入快照在请求前后复验，预算/身份缺口明确不完整。
- [x] 71 项 LLM 契约/HTTP/证据测试、提取包 4 项真实跨包验收；两次授权 `.env` 请求使用同一报告对照，源码引用通过但不据此宣称整体精度提升。见[实验](experiments/llm_evidence/README.md)。平台产物验收已接入 CI。
- [ ] 后续：按标注语料统计复核质量，再考虑受预算约束的主动补查与独立风险候选发现；本轮不承诺它们已实现。

## 2026-09-26：编译上下文与绑定成本

- [x] 编译计划结构化为生产/白盒/黑盒单位；保留旧文件并集用于语法范围，生产 IR 有显式源码与单位映射。
- [x] 修复普通 `.mbt` 内测试回调的生产误报；相关 `#cfg` 显式不完整，不自行猜测条件结果。
- [x] 函数体先做结构转换，通过后才查询绑定；不支持的 260 调用回调只查询注册点一次，原有查询/时间预算不变。
- [x] 独立复核补齐包类型、依赖未知/重写编译参数与快照覆盖检查；绝对包路径按本机路径规则规范化。
- [x] 本地 7 组新编译上下文验收、四后端单元、37 项搬迁交付、22 项 CLI、4 项进程监督通过；三平台门禁已接入 CI，最终交付记录见实验目录。
...
- [x] M1：统一生产 `.mbt` 输入的模块、包/包类型、后端、编译角色及源码/函数映射；测试源由编译器角色分离，内联 `test` 不进入生产语义。条件声明、未知编译上下文和快照缺口明确拒绝。见[编译上下文实验](experiments/compilation_context/README.md)。
...
- [x] P1a：已有语义告警复核包纳入报告调用/单位身份、IR、完整函数源码与声明模型；不凭模型补绑定。
- [ ] P1b：为普通语法告警补充可核实的 API 行为证据；外部库运行语义仍须独立核实。
