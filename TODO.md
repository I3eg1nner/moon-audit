...
- [ ] 下一阶段 M1：保留编译命令中的生产/黑盒/白盒角色，定义 `SourceUnit`（原文件、模块、包、后端、角色、前端 profile、位置映射）；先评估官方提取接口，缺失时选择完整 Markdown 解析基础设施，避免手写围栏特判。
- [ ] M2：实现有限 literate profile 并对两代编译器差异验收；未知 profile/语法明确不完整。核心验收：原始文件不变、位置准确、忽略展示块、区分 check/test、保持真实项目既有结果。
- [ ] M3：模块发现→逐模块验证→聚合工作区报告；之后再扩展经核实的数据流模型。当前多模块语法归属不代表工作区验证完成。
...
- [x] P0 调研：两代 `.mbt.md` 块标签、测试角色和原始坐标已建立实验契约；提取适配仍未实现，转上方 M1/M2，core/actrun 保持不完整。
